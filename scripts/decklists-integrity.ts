...
import { SEAT_MATCH_PRECISION_THRESHOLD, scoreAgainstSeat, formatPct } from "./lib/deckMatching";
...
async function main() {
  ...
  const isDraftComplete = (draftId: string) =>
    isCompletedForStats(draftPhaseById.get(draftId) ?? "");
  ...
  const lowCoverage = scored
    .filter(
      (s) => s.precision >= SEAT_MATCH_PRECISION_THRESHOLD && s.coverage < COVERAGE_FLAG_THRESHOLD
    )
  ...
  for (const reason of ["missing", "draft-never-collected", "in-progress"] as const) {
    ...
    const label =
      reason === "in-progress" ? "in-progress (draft not complete, not actionable)" : reason;
    console.log(
      `  ${label} (${group.length}): ${group.map((a) => `${a.draftId}:${a.seat}`).join(", ")}`
    );
  }
