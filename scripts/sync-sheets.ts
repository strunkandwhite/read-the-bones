...
import { fetchDraftFromSheet, TAB_NAMES } from "../src/build/sheets";
...
async function syncDraft(
  ...
  try {
    ...
    if (tabsSynced.length === 0) {
      console.log(`  All tabs unchanged`);
      return { status: "unchanged", tabsSynced };
    }
...
async function main() {
  ...
  for (let i = 0; i < drafts.length; i++) {
    ...
    if (i < drafts.length - 1 && !dryRun && (result.status === "synced" || result.status === "unchanged")) {
