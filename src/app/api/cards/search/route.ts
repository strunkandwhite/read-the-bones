...
export const GET = withApiErrors(async (request: NextRequest) => {
  ...
  if (availableOnly && !draftId) {
    return NextResponse.json({ error: "available_only requires draft_id" }, { status: 400 });
  }
  if (beforePickN !== undefined && !availableOnly) {
    return NextResponse.json({ error: "before_pick_n requires available_only" }, { status: 400 });
  }
