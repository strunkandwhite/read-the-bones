...
import { getFloatedCards, addFloatedCard, removeFloatedCard } from "@/core/db/queries/floatedCards";
...
export const GET = withApiErrors(async (request: NextRequest, { params }: Params) => {
...
export const PUT = withApiErrors(async (request: NextRequest, { params }: Params) => {
  const { id: draftId } = await params;
  const client = await getClient();
  const { seat } = await authenticateSeat(client, request, draftId);
...
export const DELETE = withApiErrors(async (request: NextRequest, { params }: Params) => {
  const { id: draftId } = await params;
  const client = await getClient();
  const { seat } = await authenticateSeat(client, request, draftId);
