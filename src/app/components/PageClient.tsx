...
export function PageClient({
  ...
  const { deckBuilderModalOpen, setDeckBuilderModalOpen, draftBoardOpen, setDraftBoardOpen } =
    useModalManagement({
      activeDraft,
      selectedSeat,
    });
  ...
  return (
    <div className="min-h-dvh bg-zinc-50 dark:bg-zinc-950">
      ...
      <div
        className={`mx-auto px-4 pt-4 pb-0 sm:px-6 lg:px-8 ${
      ...
      >
        ...
        <div className="mb-3 flex items-center gap-3">
          ...
          <div className="flex shrink-0 items-center gap-3">
            ...
            />
            <h1 className="hidden text-xl font-bold tracking-tight text-zinc-900 sm:block dark:text-zinc-100">
              Read the Bones
            </h1>
          </div>
          ...
          <div className="relative min-w-0 flex-1">
            ...
            <input
              ...
              className="w-full rounded-lg border border-zinc-300 bg-white py-1.5 pr-8 pl-3 text-sm text-zinc-900 placeholder-zinc-500 focus:border-transparent focus:ring-2 focus:ring-blue-500 focus:outline-none dark:border-zinc-600 dark:bg-zinc-800 dark:text-zinc-100 dark:placeholder-zinc-400"
            />
            {searchQuery && (
              <button
                type="button"
                onClick={clearSearch}
                className="absolute top-1/2 right-2 -translate-y-1/2 rounded p-0.5 text-zinc-400 hover:text-zinc-600 focus:ring-2 focus:ring-blue-500 focus:outline-none dark:text-zinc-500 dark:hover:text-zinc-300"
          ...
          <div className="group relative hidden sm:block">
            ...
            <div className="absolute top-full left-1/2 z-50 mt-2 hidden w-64 -translate-x-1/2 rounded-lg border border-zinc-200 bg-white p-3 text-sm shadow-lg group-hover:block dark:border-zinc-700 dark:bg-zinc-800">
              ...
              <ul className="space-y-1 text-zinc-600 dark:text-zinc-300">
                ...
                <li>
                  <code className="rounded bg-zinc-100 px-1 text-xs dark:bg-zinc-700">c:r</code>{" "}
                  color (w/u/b/r/g/c/m)
                </li>
                <li>
                  <code className="rounded bg-zinc-100 px-1 text-xs dark:bg-zinc-700">c=ub</code>{" "}
                  exact colors
                </li>
                <li>
                  <code className="rounded bg-zinc-100 px-1 text-xs dark:bg-zinc-700">id:ubr</code>{" "}
                  color identity
                </li>
                <li>
                  <code className="rounded bg-zinc-100 px-1 text-xs dark:bg-zinc-700">m:GG</code>{" "}
                  mana cost
                </li>
                <li>
                  <code className="rounded bg-zinc-100 px-1 text-xs dark:bg-zinc-700">mv=3</code>{" "}
                  <code className="rounded bg-zinc-100 px-1 text-xs dark:bg-zinc-700">
                    mv&lt;=2
                  </code>{" "}
                  mana value
                ...
                <li>
                  <code className="rounded bg-zinc-100 px-1 text-xs dark:bg-zinc-700">-t:land</code>{" "}
                  negation
                </li>
                <li>
                  <code className="rounded bg-zinc-100 px-1 text-xs dark:bg-zinc-700">or</code>{" "}
                  <code className="rounded bg-zinc-100 px-1 text-xs dark:bg-zinc-700">( )</code>{" "}
                  logic
                </li>
              </ul>
              <div className="mt-2 text-xs text-zinc-500 dark:text-zinc-400">
                <code className="rounded bg-zinc-100 px-1 dark:bg-zinc-700">t:instant c:u</code> =
                AND,{" "}
                <code className="rounded bg-zinc-100 px-1 dark:bg-zinc-700">
                  (t:instant or t:sorcery) c:u
                </code>{" "}
                = grouped OR
              </div>
            </div>
          ...
          <div className="hidden h-5 w-px bg-zinc-300 lg:block dark:bg-zinc-600" />
              ...
              <button
                ...
                title={
                  isMyTurn
                    ? "Your Pick!"
                    : selectedSeat !== null
                      ? `${activeDraft}, Seat ${selectedSeat}`
                      : `Pod View — ${activeDraft}`
                }
                aria-label={
                  isMyTurn
                    ? "Your Pick!"
                    : selectedSeat !== null
                      ? `Pod View — ${activeDraft}, Seat ${selectedSeat}`
                      : `Pod View — ${activeDraft}`
                }
              >
        ...
        {deckFilteredCards.length > 0 ? (
          <CardTable cards={deckFilteredCards} onCardClick={handleCardClick} />
        ) : (
