 ...
 * - Top player selection (persisted to localStorage)
 * - Top player weighting toggle (persisted to localStorage)
 */
export function PageClient({
  ...
  const [storedTopPlayers, setStoredTopPlayers] = useLocalStorage<string[]>(
    STORAGE_KEY_TOP_PLAYERS,
    []
  );
  const [useTopPlayerWeighting, setUseTopPlayerWeighting] = useLocalStorage<boolean>(
    STORAGE_KEY_USE_WEIGHTING,
    false
  );
  ...
  // Filter stored players to only include valid players from current data
  const topPlayers = storedTopPlayers.filter((p) => initialPlayers.includes(p));
  ...
  // Update stored players, filtering to valid players
  const handleTopPlayersChange = useCallback(
    (players: string[]) => {
      const validPlayers = players.filter((p) => initialPlayers.includes(p));
      setStoredTopPlayers(validPlayers);
    },
    [initialPlayers, setStoredTopPlayers]
  );

  // Toggle weighting
  const handleWeightingToggle = useCallback(
    (enabled: boolean) => {
      setUseTopPlayerWeighting(enabled);
    },
    [setUseTopPlayerWeighting]
  );
