...
describe("authenticateSeat", () => {
  ...
  it("throws for missing token", async () => {
    ...
    await expect(authenticateSeat(client, req, "draft-1")).rejects.toThrow("Missing seat token");
  });
  ...
  it("throws for invalid token (resolveToken returns null)", async () => {
    ...
    await expect(authenticateSeat(client, req, "draft-1")).rejects.toThrow("Invalid seat token");
  });
