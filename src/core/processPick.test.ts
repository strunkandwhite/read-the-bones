...
describe("processPick", () => {
  ...
  it("rejects when phase is not drafting", async () => {
    ...
    await expect(processPick(mockClient as never, baseInput)).rejects.toThrow(
      "Draft is in 'complete' phase, not 'drafting'"
    );
  });
  ...
  it("throws NotFoundError when draft does not exist", async () => {
    ...
    await expect(processPick(mockClient as never, baseInput)).rejects.toThrow("Draft not found");
  });
  ...
  it("rejects when it's not this seat's turn", async () => {
    ...
    await expect(processPick(mockClient as never, { ...baseInput, seat: 2 })).rejects.toThrow(
      "It's seat 1's turn, not seat 2's"
    );
  });
  ...
  it("rejects when all picks are already made", async () => {
    ...
    await expect(processPick(mockClient as never, baseInput)).rejects.toThrow("All picks are made");
  });
  ...
  it("rejects card not in this draft's cube (zero rows from cube_snapshot_cards)", async () => {
    ...
    await expect(processPick(mockClient as never, baseInput)).rejects.toThrow(
      "Counterspell is not in this draft's cube"
    );
  });
  ...
  it("rejects already-picked card", async () => {
    ...
    await expect(processPick(mockClient as never, baseInput)).rejects.toThrow(
      "Counterspell has already been picked"
    );
  });
  ...
  it("throws ConflictError when optimistic INSERT finds pick_n already taken", async () => {
    ...
    await expect(processPick(mockClient as never, baseInput)).rejects.toThrow(
      "Conflict: pick_n already exists — retry"
    );
  });
  ...
  it("records pick and returns it", async () => {
    ...
    expect(result).toEqual({
      picks: [{ pickN: 1, seat: 1, cardId: 42, cardName: "Counterspell" }],
      phaseChanged: false,
      newPhase: null,
    });
  });
  ...
  it("calls removeFloatedCardByCardId when isLastCopy is true", async () => {
    ...
    expect(removeFloatedCardByCardId).toHaveBeenCalledWith(mockClient, "draft-1", 42);
  });
  ...
  it("transitions to playing when all picks are made", async () => {
    ...
    expect(result).toEqual({
      picks: [{ pickN: 24, seat: 4, cardId: 99, cardName: "Final Card" }],
      phaseChanged: true,
      newPhase: "playing",
    });
    ...
    const clearQueueCall = calls.find(
      (c: unknown[]) =>
  ...
  describe("queue cleanup on last copy", () => {
    it("disables auto-pick for seats returned in pauseSeats when last copy is taken", async () => {
      ...
      vi.mocked(getAllSeatSettings).mockResolvedValueOnce(
        new Map([[2, { autoPick: true, displayName: null }]])
      );
  ...
  describe("multi-copy cards", () => {
    it("allows picking a 2-copy card that has been picked once", async () => {
      ...
      mockClient.execute.mockResolvedValueOnce(createQueryResult([{ latest: 1 }]));
      ...
      mockClient.execute.mockResolvedValueOnce(createQueryResult([{ picked_count: 2, qty: 2 }]));
    ...
    it("rejects picking a 2-copy card when both copies are taken", async () => {
      ...
      mockClient.execute.mockResolvedValueOnce(createQueryResult([{ latest: 2 }]));
      // 3. Availability check -- picked_count=2, qty=2 (fully taken)
      mockClient.execute.mockResolvedValueOnce(createQueryResult([{ picked_count: 2, qty: 2 }]));

      await expect(processPick(mockClient as never, { ...baseInput, seat: 3 })).rejects.toThrow(
        "Counterspell has already been picked"
      );
    });
    ...
    it("calls trimExcessQueueEntries when pick is not last copy", async () => {
      ...
      mockClient.execute.mockResolvedValueOnce(createQueryResult([{ picked_count: 0, qty: 3 }]));
      // 4. INSERT pick_events -- success
      mockClient.execute.mockResolvedValueOnce(createQueryResult([], 1));
      // 5. Post-insert copy-check re-query (picked_count now 1, qty 3 -> 2 remain)
      mockClient.execute.mockResolvedValueOnce(createQueryResult([{ picked_count: 1, qty: 3 }]));
  ...
  describe("auto-pick cascade", () => {
    ...
    it("rejects a cascaded pick whose card is not in the cube (S1 cascade path)", async () => {
      ...
      await expect(processPick(mockClient as never, baseInput)).rejects.toThrow(
        "Off Cube Card is not in this draft's cube"
      );
    });
    ...
    it("stops at maxCascade: loop exits after numSeats*2 iterations", async () => {
      ...
      vi.mocked(getAllSeatSettings).mockResolvedValue(
        new Map([
          [1, { autoPick: true, displayName: null }],
          [2, { autoPick: true, displayName: null }],
        ])
      );
      vi.mocked(getAutoPickCandidate).mockResolvedValue({
        kind: "candidate",
        cardId: 77,
        entryIndex: 0,
      });
...
describe("triggerAutoPickOnDemand", () => {
  ...
  beforeEach(async () => {
    ...
    vi.mocked(getAllSeatSettings).mockResolvedValue(
      new Map([[1, { autoPick: true, displayName: null }]])
    );
  });
  ...
  it("picks nothing and reports autoPickDisabled when the seat has auto-pick off", async () => {
    ...
    const { getAutoPickCandidate } = await import("./db/queries/pickQueue");
    vi.mocked(getAllSeatSettings).mockResolvedValue(
      new Map([[1, { autoPick: false, displayName: null }]])
    );
  ...
  it("throws NotFoundError when draft does not exist", async () => {
    ...
    await expect(triggerAutoPickOnDemand(mockClient as never, "draft-1", 1)).rejects.toThrow(
      "Draft not found"
    );
  });
  ...
  it("throws ValidationError when it is not this seat's turn", async () => {
    ...
    await expect(triggerAutoPickOnDemand(mockClient as never, "draft-1", 2)).rejects.toThrow(
      "It's seat 1's turn, not seat 2's"
    );
  });
  ...
  it("picks the same card the cascade would for an identical queue state", async () => {
    ...
    vi.mocked(fulfillGroupEntry).mockResolvedValue({
      mode: "pause",
      cards: [{ id: 42, name: "Counterspell" }],
    });
    ...
    expect(getAutoPickCandidate).toHaveBeenCalledWith(mockClient, "draft-1", 1, new Set([42]));
  });
