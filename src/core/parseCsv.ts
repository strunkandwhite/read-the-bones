...
export function parseDraftPicks(
  csvContent: string,
  draftId: string
): { picks: CardPick[]; numDrafters: number } {
  const result = Papa.parse<string[]>(csvContent, {
    skipEmptyLines: false,
  });
  ...
  const rows = result.data;
  if (rows.length < 4) {
    return { picks: [], numDrafters: 0 }; // Not enough rows for valid data
  }
  ...
  for (let rowIndex = 3; rowIndex < rows.length; rowIndex++) {
    ...
    for (let drafterIndex = 0; drafterIndex < numDrafters; drafterIndex++) {
      ...
      picks.push({
        ...
        wasPicked: true,
        draftId,
        drafterName: drafterNames[drafterIndex],
        color,
      });
    }
  }

  return { picks, numDrafters };
}
...
export function parseDraft(
  ...
  poolCsv: string,
  draftId: string
): { picks: CardPick[]; poolSize: number; numDrafters: number } {
  // Parse the picks first
  const { picks, numDrafters } = parseDraftPicks(picksCsv, draftId);
  ...
  // and a drafter name of "Unpicked"
  ...
  return { picks, poolSize, numDrafters };
}
