...
export function parseDraftPicks(
  csvContent: string,
  draftId: string
): { picks: CardPick[]; numDrafters: number; drafterNames: string[]; doublePickStartsAfterRound: number | null } {
  const result = Papa.parse<string[]>(csvContent, {
    skipEmptyLines: false,
  });
  ...
  const rows = result.data;
  if (rows.length < 4) {
    return { picks: [], numDrafters: 0, drafterNames: [], doublePickStartsAfterRound: null }; // Not enough rows for valid data
  }
  ...
  if (numDrafters === 0) {
    return { picks: [], numDrafters: 0, drafterNames: [], doublePickStartsAfterRound: null };
  }

  // Extract "Double Picks After:" metadata from the CSV
  // This appears in the right-side metadata columns (e.g., row for round 2)
  ...
  for (let rowIndex = 3; rowIndex < rows.length; rowIndex++) {
    ...
    for (let drafterIndex = 0; drafterIndex < numDrafters; drafterIndex++) {
      ...
      if (doublePickStartsAfterRound === null || roundNumber <= doublePickStartsAfterRound) {
        ...
        // Double-pick mode: pairs of CSV rows form one "double round" with 2 picks per drafter.
        // Each drafter picks twice consecutively before moving to the next.
        ...
        // If threshold is odd (e.g., 25), last standard round was forward, so first double round is reverse
        const isReverse = doublePickStartsAfterRound % 2 === 1
        ...
        pickPosition = basePosition + drafterOrderIndex * 2 + isSecondPickInPair;
      }
  ...
  return { picks, numDrafters, drafterNames, doublePickStartsAfterRound };
}
...
export function parseDraft(
  ...
  poolCsv: string,
  draftId: string
): { picks: CardPick[]; poolSize: number; numDrafters: number; drafterNames: string[]; doublePickStartsAfterRound: number | null } {
  // Parse the picks first
  const { picks, numDrafters, drafterNames, doublePickStartsAfterRound } = parseDraftPicks(picksCsv, draftId);
  ...
  return { picks, poolSize, numDrafters, drafterNames, doublePickStartsAfterRound };
}
