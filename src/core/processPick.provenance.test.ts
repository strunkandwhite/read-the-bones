...
async function seed(): Promise<Client> {
  ...
  await insertSeatToken(client, DRAFT, 1, {
    autoPick: true,
    queueJson: queueOf([[11, "S1 First"]]),
  });
  await insertSeatToken(client, DRAFT, 2, {
    autoPick: true,
    queueJson: queueOf([[21, "S2 First"]]),
  });
  await insertSeatToken(client, DRAFT, 3, {
    autoPick: true,
    queueJson: queueOf([[31, "S3 First"]]),
  });
  return client;
}
...
describe("sheet-sourced picks", () => {
  it("batchInsertPicks stamps source=sheet and a non-null created_at", async () => {
    const client = await createMemDb();
    await batchInsertPicks(client, [{ draftId: DRAFT, pickN: 1, seat: 1, cardId: 1 }]);
    const r = await client.execute({
