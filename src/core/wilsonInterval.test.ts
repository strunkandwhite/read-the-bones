...
describe("wilsonInterval", () => {
  it("should return { lower: 0, center: 0, upper: 0 } for zero trials", () => {
  ...
  it("should include a center field between lower and upper", () => {
    const { lower, center, upper } = wilsonInterval(7, 10);
    expect(center).toBeGreaterThan(lower);
    expect(center).toBeLessThan(upper);
  });

  it("should round values to 3 decimal places", () => {
    const { lower, center, upper } = wilsonInterval(3, 7);
    // Check that each value has at most 3 decimal places
    for (const val of [lower, center, upper]) {
